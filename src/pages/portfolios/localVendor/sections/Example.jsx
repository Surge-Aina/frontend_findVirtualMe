...
const AboutSection = () => {
  ...
  const handleAddImage = async (e) => {
    ...
    try {
      ...
      await API.put("/about", { gridImages: newImages }); // persist
    } catch (err) {
  ...
  const handleDeleteImage = async (src) => {
    ...
    setBottomImages(newImages);
    try {
      await API.put("/about", { gridImages: newImages }); // persist list without the image
      // (If you later want to delete the file from disk too, add a backend DELETE route.)
    } catch (err) {
  ...
  return (
    ...
    >
      {/* Two-column content — clean cards, no section labels */}
      ...
      {/* Masonry gallery with admin-only delete */}
      <motion.div
      ...
      >
        ...
        <div className="columns-1 sm:columns-2 gap-4 [column-fill:_balance]">
          {bottomImages.map((src) => (
            <figure
              key={src}
              className="group mb-4 break-inside-avoid rounded-2xl overflow-hidden shadow-sm relative"
            >
              {/* image */}
              <img
                src={src}
              ...
              {/* admin-only delete overlay */}
              {isAdminLoggedIn() && (
                <button
                  onClick={() => handleDeleteImage(src)}
                  className="absolute top-2 right-2 px-2.5 py-1.5 rounded-md text-xs font-medium bg-white/80 backdrop-blur hover:bg-white shadow transition opacity-0 group-hover:opacity-100"
                  title="Remove image"
                >
                  Delete
                </button>
          ...
          {Array.from({
            length: Math.max(0, MAX_IMAGES - bottomImages.length),
          }).map((_, i) => (
            <div
              key={`empty-${i}`}
              className="mb-4 break-inside-avoid rounded-2xl bg-slate-100 aspect-[4/3]"
            />
          ))}
        </div>
      </motion.div>
